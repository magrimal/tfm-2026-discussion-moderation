...
The project uses six milestones (table~\ref{tab:milestones-en}) covering problem exploration, design and implementation, inputs and outputs, technical evaluation, and deployment. Utility evaluation with students in a real educational setting remains outside the scope.
...
In practice, the work is organized in discovery cycles, so the findings from each phase inform the next. Tasks, the GitHub board, and Architecture Decision Records (ADRs), available in the project repository, document the exploration and design decisions. Milestones delimit the phases, and each phase ends when the questions that define it have been answered.
