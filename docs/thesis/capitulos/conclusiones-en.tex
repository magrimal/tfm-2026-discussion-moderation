...
\item Model-reported confidence was not calibrated against observed outcomes and is not interpreted as a reliable measure of quality. Reproducibility of outputs under the same configuration was not studied systematically.
\item Platform independence was examined with local files and an Open edX proof of concept. It was not verified through a second integration with another LMS or forum, and the event-activation mechanism was not tested broadly enough to establish operational reliability. The publication function was tested with simulated components and does not include prior instructor review, so it is not considered safe for automatic publication.
